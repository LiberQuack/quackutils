...
exports.default = new plugin_1.Resolver({
    async resolve(metaData) {
        const exp = /.js$/;
        if (exp.test(metaData.specifier)) {
            const [err, fsStat, tsRealPath] = await new Promise(resolve => {
                const baseDir = metaData.dependency.sourcePath.replace(/[\w-]+?\.[jt]sx?/, "");
                const relativeTsPath = metaData.specifier.replace(exp, ".ts");
                const absoluteTsPath = path_1.default.resolve(baseDir + relativeTsPath);
                fs.stat(absoluteTsPath, {}, (err, stats) => {
                    if (err) {
                        fs.stat(absoluteTsPath + "x", {}, (err, stats) => {
                            resolve([err, stats, absoluteTsPath + "x"]);
                        });
                    }
                    else {
                        resolve([err, stats, absoluteTsPath]);
                    }
                });
            });
            if (fsStat) {
